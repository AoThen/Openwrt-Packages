...
broadband_init() {
	...
	# 检查外部调用工具
	command -v wget >/dev/null || { _log "GNU Wget 未安装,尝试安装中...请重启插件"; opkg update; opkg install wget-ssl; return 3; }

	# opkg find wget-ssl || { opkg update; opkg install wget-ssl; _log "GNU Wget-ssl 未安装,尝试安装中..."; }
